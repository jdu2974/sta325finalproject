...
First, we  where we determine the features that are important (XgBoost to fit gradient-boosted decision trees),
...
(i.e. not going for fourth down on fourth and 20 or when the team with posession
...
as.factor(boosting_f_train$play_type)
...
We use the same gradient-boosted model on our test dataset of 2020 plays. First,
we plot the confusion matrix for non fourth down plays, where the error rate is
41%. We notice that while the model is very good at classifying punt situations
