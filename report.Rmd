...
possession eventually won the game for fourth down plays). We examine if the
...
kable(freq_nf) %>% kable_styling(latex_options = "HOLD_position", full_width = F)
```
...
kable(freq_f) %>% kable_styling(latex_options = "HOLD_position", full_width = F)
```
...
$$log(\frac{\pi_i}{1-\pi_i}) = \beta_0 + \beta_1{\text{Pressure}} + \beta_2{\text{Wind Speed}} + \beta_3{\text{Humidity}} + \beta_4{\text{Temperature}} + \beta_5{\text{Precipitation}} + \beta_6{\text{Dewpoint}} + \beta_7{\text{Run}}  + \sum_{j = 8}^{18}\beta_jX_j$$

In our regression, the pass play type is the base group.

$\sum_{j = 8}^{18}\beta_jX_j$ indicates all predictor variables not related to play type or weather that were significant from the tree-based model (i.e. Yard Line, Yards to Go for a First Down, Time Remaining in the Half, etc). 
...
First, we run a similar model where we determine the features that are important,
...
(i.e. not going for fourth down on fourth and 20).
...
From the feature importance graph, we find that Humidity, Pressure, and Temperature 
are among the 10 have some of the highest feature scores (as quantified by Gain), 
but the predominant variable in
terms of feature importance is the current yard line position and yards to go.
...
as a proxy for measuring success of plays. We fit the model with all predictors 
...
$$log(\frac{\pi_i}{1-\pi_i}) = \beta_0 + \beta_1{\text{Pressure}} + \beta_2{\text{Wind Speed}} + \beta_3{\text{Humidity}} + \beta_4{\text{Temperature}} + \beta_5{\text{Precipitation}} + \beta_6{\text{Dewpoint}} + \beta_7{\text{Run}} + \beta_8{\text{Pass}} + \beta_9{\text{Punt}}  + \sum_{j = 10}^{18}\beta_jX_j$$
...
$\sum_{j = 10}^{18}\beta_jX_j$ indicates all predictor variables not related to play type or weather that were significant from the tree-based model (i.e. Yard Line, Yards to Go for a First Down, Time Remaining in the Half, etc). 
...
controlling for the type of play in the particular game scenario. 
There is a disconnect between the perceived influence of weather on a coach's decision
to run or pass and the impact of weather on successful play outcomes. We determine
