...
Now, we run a logistic regression model to determine if weather has a significant effect on play outcomes. Instead of using series success rate as the response variable for this model, we choose to use the result of the game (1 if the possession team wins and 0 if the possession team loses) as a proxy for measuring success of plays. 
...
From the model, we can see that weather has no significant impact on the outcome of plays in terms of their ability to increase or decrease win probability, holding all other factors equal. This is indicated by the fact that all weather related variables have p-values > .05. The interpretation of both our tree based model and our logistic regression model in terms of our project objects is that while weather does not have an impact on coaching decision-making on 4th downs, it means that coaches are probably correct in that weather should not have an impact on coaching decision-making on 4th downs.