...
likely to experience it repeatedly. Yet in practice, coaches will often revert
to decision making that has worked in the past in situations that objectively 
do not justify such a decision.

We believe that NFL coaches also experience bias
...
the distribution of 4th down play outcomes. There are 10 NFL stadiums that are
domed, with 5 having retractable roofs. The play-by-play dataset has four possible
...
game to be equivalent to a non-retractable dome stadium. A closed roof stadium
will allow for specific climate-control, reducing the influence of external temperature, humidity,
and wind speed to essentially 0.
...
so we perform preprocessing through one-hot-encoding.
...
```{r boost_nf, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
...
```{r glm_nf, echo = FALSE, eval = FALSE, message = FALSE, warning = FALSE}
rel_df <- training_nf_data  %>%
  filter(play_type == "pass" | play_type == "run") %>%
  filter(roof == "outdoors")

rel_df <- rel_df[, c(xgb_cols, "play_type", "series_success")]

form1 <- glm(series_success ~., data = rel_df, family = "binomial")
summary(form1)
```

At an alpha level of .05, we observe that pressure, humidity, dewpoint, and 
temperature are not statistically significant, though wind speed is. There is
a disconnect between the perceived influence of weather on a coach's decision
