---
title: "Examining the Imapct of Weather on NFL Decision-Making"
...
$$log(\frac{\pi_i}{1-\pi_i}) = \beta_0 + \sum_{j = 1}^{6}\beta_jX_j + \beta_7{\text{Run}}  + \sum_{j = 8}^{23}\beta_jX_j$$
...
$\sum_{j = 8}^{23}\beta_jX_j$ indicates all predictor variables not related to play type or weather that were significant from the tree-based model (i.e. Yard Line, Yards to Go for a First Down, Time Remaining in the Half, etc). 
...
$$log(\frac{\pi_i}{1-\pi_i}) = \beta_0 + \sum_{j = 1}^{6}\beta_jX_j + \beta_7{\text{Run}} + \beta_8{\text{Pass}} + \beta_9{\text{Punt}}  + \sum_{j = 10}^{18}\beta_jX_j$$
...
```{r boost_nf, message = FALSE, warning = FALSE, echo = FALSE, results='hide',eval = FALSE}
