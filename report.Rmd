---
title: "Examinng the Imapct of Weather on NFL Decision-Making"
output:
...
for the decisions made over the course of game that ultimately lead to a win or loss.
...
their judgement and decision making when baselined against a baseline of rationality.
...
support the notion that athletes experiencing a positive outcomes are more 
...
One key consideration that consistently affects game-planning and subsequently
decision-making are the weather conditions. For stadiums that have an open roof
(there are 22 outdoor stadiums an additional 5 domed stadiums with retractable
...
of a completion, while high humidity results in less dense air that allows footballs
...
down (first down and touchdown for non fourth down plays, ___ for fourth down
plays).
...
We first explore the significant of the 112,381 with respect to the current
...
From these plays, we consider how playcalling on first, second, and third downs vary versus fourth
downs. We display the number of occurrences of each play type among non-fourth
down plays versus fourth down plays from our training datasets. First, for non
fourth down plays:
...
to an outdoors stadium and a domed stadium that has a closed roof for the particular
game to be equivalent to a non-retractable dome stadium. 

```{r, echo = FALSE, fig.height = 7.5, fig.width = 16, fig.cap = "Distribution of  4th Down Plays Based on Type of Stadium"}
...
See appendix for full list of variables. 
...
of 10 and 100 rounds. Here we plot the importance feature graph from the model.
...
xgboost_train = xgb.DMatrix(data= as.matrix(one_hot), label= boosting_nf_train$play_type)
...
we find that Pressure, Humidity, DewPoint, and Temperature have some of the 
highest feature scores (as quantified by Gain), indicating that such conditions 
...
We now include play type as a predictor
to control for differences in run and pass plays. We display the estimates
for our weather predictors.
...
and precipitation are not statistically significant, though wind speed is, even after
controlling for type of play. There is a disconnect between the perceived influence of weather on a coach's decision
to run or pass and the impact of weather on successful play outcomes.
Backwards selection with our full model resulted in only temperature, dewpoint,
week of game, and seconds remaining in game variables being removed.

### Testing

We use the same gradient-boosted model on our test dataset of 2020 plays, and we
find that 

```{r boost_nf, message = FALSE, warning = FALSE, echo = FALSE, results='hide'}
...
Here we plot the confusion matrix, where the error rate is 35%.
...
one_hot <- one_hot(setDT(boosting_f_train[, relevant_cols]) )



xgboost_train = xgb.DMatrix(data= as.matrix(one_hot), label= as.factor(boosting_f_train$play_type))
...
Now, we run a logistic regression model to determine if weather has a significant effect on play outcomes. Instead of using series success rate as the response variable for this model, we choose to use the result of the game (1 if the possession team wins and 0 if the possession team loses) as a proxy for measuring success of plays. Below is the full model

$$Pr(\text{Outcome}_i = \text{Win}) = \pi_i$$
$$log(\frac{\pi_i}{1-\pi_i}) = \beta_0 + \beta_1{\text{Pressure}} + \beta_2{\text{Wind Speed}} + \beta_3{\text{Humidity}} + \beta_4{\text{Temperature}} + \beta_5{\text{Precipitation}} + \beta_6{\text{Dewpoint}} + \beta_7{\text{Run}} + \beta_8{\text{Pass}} + \beta_9{\text{Punt}}  + \sum_{j = 8}^{p}\beta_jX_j$$
...
$\sum_{j = 8}^{p}\beta_jX_j$ indicates all predictor variables not related to play type or weather that were significant from the tree-based model (i.e. Yard Line, Yards to Go for a First Down, Time Remaining in the Half, etc). 

```{r}
xgb_cols <- (xgb_f %>% filter(Gain >= .05))$Feature
...
f <- summary(nf_model)
f <- nf$coefficients[c("Pressure","Humidity", "DewPoint", "Temperature", "WindSpeed", "Precipitation"), ]
f <- as.data.frame(nf)
...
From the model, we can see that the only weather variable that has a significant impact on play outcomes (in terms of wins and losses) is wind speed. This is indicated by the fact that it is the only weather variable that has a p-value < .05. 
